...
def create_gru_network_for_training(input_shape, total_cells, initial_bias, show_summary=True):
    ...
    model = tf.keras.models.Sequential([
                                         ...
                                         tf.keras.layers.Bidirectional(tf.keras.layers.GRU(total_cells, return_sequences=True, recurrent_dropout=0.2)),
                                         tf.keras.layers.Dropout(0.2),
                                         tf.keras.layers.BatchNormalization(),
                                         tf.keras.layers.Bidirectional(tf.keras.layers.GRU(total_cells)),
...
def train_and_test_for_user(user,
                            ...
                            epochs,
                            snr_db,
                            filter_outliers_first
                           ):

    # get data which will be used during training
    train_data_pos = boun.get_train_data(user, dataset, filter_outliers=filter_outliers_first).values
    ...
    if not filter_outliers_first:
        train_data_pos = boun.rmoutliers(train_data_pos)
...
def train_and_test_users(dataset,
    ...
    for k in range(subject_count):
        ...
        i_eer, i_zmr, i_acc = train_and_test_for_user(user = s,
                                                      ...
                                                      epochs=epochs,
                                                      snr_db=snr_db,
                                                      filter_outliers_first=filter_outliers_first
                                                    )
        eer[k] = i_eer
        zmr[k] = i_zmr
        acc[k] = i_acc
    ...
    boun.add_to_log('Results for epochs = {:.3f} SNR_db: {} filter_outliers_first: {} test_imposter_count = {:.3f} total_cells = {:.3f} limit_user_count = {:.3f} test_average_user_count = {:.3f} batchsize = {:.3f}'.format(epochs, snr_db, filter_outliers_first, test_imposter_count, total_cells, limit_user_count, test_average_user_count, batchsize))
...
def main():
    ...
    pure_data = boun.load_pure_data_csv()
    cmu_data = boun.pure_to_cmu(pure_data)

    eer, zmr = train_and_test_users(dataset = cmu_data,
                                    test_imposter_count = 5,
                                    total_cells = 32,
                                    limit_user_count = -1,
